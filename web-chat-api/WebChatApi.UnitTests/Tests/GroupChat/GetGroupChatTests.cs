...
public class GetGroupChatTests : BaseTest
{
	[Fact]
	public async Task GetByIdAsync_Should_ReturnSuccessResponse()
	{
		...
		//Act
		var res = await service.GetByIdAsync(groupChatId);

		//Assert
		res.Success.Should().BeTrue();
		res.ErrorMessage.Should().BeNullOrEmpty();
		var groupChat = res.Payload as GroupChatDto;
		groupChat.Should().NotBeNull();
		groupChat.Should().Match<GroupChatDto>(g =>
			g.Id == groupChatId &&
			g.Name == createGroupChatDto.Name &&
			g.Description == createGroupChatDto.Description &&
			g.CreatorId == createGroupChatDto.CreatorId &&
			g.CreatedAt != default
		);
	}
